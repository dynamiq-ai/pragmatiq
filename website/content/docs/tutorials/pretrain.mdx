...
`pragmatiq pretrain` defaults to `--config auto`, and `--show-config` prints the resolved
plan without training. Batches are collated on a prefetch thread so the GPU never waits on
the host. A one-line heartbeat reports `step, loss, mlm_acc, tokens/s, ETA`.
