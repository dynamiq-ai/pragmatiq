---
title: Quickstart
description: Install pragmatiq and run the full pipeline — generate data, tokenize, pretrain, embed, and probe — on a GPU or a laptop CPU.
---
...
This runs the entire pragmatiq pipeline end to end on synthetic data. By the end you will
have a trained model and a credit-risk probe score measured against a raw-count baseline —
proof the embedding carries signal beyond trivial counts.

<Callout title="Requirements">
Python **3.11+**, torch **2.6+**. A GPU is picked up automatically when visible (see
[Running on GPU](https://github.com/dynamiq-ai/pragmatiq#running-on-gpu)); on a CPU the
fast path below takes a few minutes in fp32. The commands use a virtual environment so the
install is isolated.
</Callout>
...
The `train` extra brings in Lightning, which the pretraining stage runs on; the plain
...
[Install](/docs/get-started/install) for the full table.
