...
| `pragmatiq info` | Versions, resolved device, CUDA, flash-attn, importable extras (`pragmatiq --version` for the version alone). |
| `pragmatiq pretrain` | Pretrain a model (`--model-size`, `--config` default `auto`, `--show-config`, `--resume auto`). |
| `pragmatiq embed` | Embed every user with a trained model → parquet. |
| `pragmatiq probe` | Probe a model on a label table (`--probe-model gbdt\|logistic\|lightgbm`, `--staleness-window 6h`). |
...
| `pragmatiq benchmark` | Throughput benchmark for embedding (`--precision auto\|bf16\|fp32`; writes `benchmark_results.md`). |
