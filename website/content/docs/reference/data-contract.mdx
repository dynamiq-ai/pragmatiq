...
| `labels/*.parquet` | optional task tables: forecasting labels use `user_id` (string), `eval_ts` (timestamp[us]), `label` (int8); AML membership uses `user_id`, `observed_through` (timestamp[us]), `label` (int8) |
...
One row per event. `source` groups schemas (e.g. `transaction`, `app`, `trading`); `fields`
...
`transfers.parquet` is the directed money-movement graph the [AML GNN](/docs/concepts/aml-graph)
runs over (optional). Forecast label tables are one row per user with an `eval_ts`;
histories are truncated at it before embedding so metrics are forecasts, not hindcasts.
AML is a graph membership task, so `labels/aml.parquet` is one row per user with
`user_id`, `observed_through`, and `label`.
