#!/usr/bin/env bash
# Acceptance check — synthetic data generator.
#
...
echo "=== throughput (100k users < 10 min on 8 cores) ==="
$PY - <<EOF
...
echo "=== credit GBDT baseline AUC in [0.70, 0.88], hard fail > 0.95 ==="
DATA="$WORK/auc_data"
...
assert 0.70 <= auc <= 0.88, f"AUC {auc} outside [0.70, 0.88] band (CI band; target ~0.72-0.85)"
prev = res["prevalence"]
...
echo ""
echo "SYNTHETIC DATA CHECKS GREEN"
